...
public final class FPU implements FingerPrintEnrollmentHandler, Runnable{
	...
	static {
		logger.info(System.getProperty("os.arch")); //$NON-NLS-1$
		if (!System.getProperty("os.arch").contains("arm")) { //$NON-NLS-1$ //$NON-NLS-2$
			//			System.loadLibrary("synergy-mock"); // changeme to //$NON-NLS-1$
			//												// "synergy"->libsynergy.so for
			//												// real device. place it in
			//												// /usr/lib/fpu/jni/.
			isMock = true;
		} else {
			System.loadLibrary("synergy2416"); // changeme to //$NON-NLS-1$
			// "synergy"->libsynergy.so for
			// real device. place it in
			// /usr/lib/fpu/jni/.
		}
	...
	public static void main(String[] args) {
		...
		FPU.getInstance().openFPU("/tmp"); //invoke the help msg for this library
		...
		int exit = 1;
		while(exit!=0){
			System.out.print("Choose: \n 0)DeviceType \n 1) Enrol \n 2) Delete \n 3) Validate \n 4) Enrol Count\n 5) Get Template \n 6) Set Template \n 7) Get Templates \n 8) Green Toggle \n 9) Red Toggle 10)Thread Toggle 0) exit \nChoice:\n");
			switch(selectionChoice()){
			case 0:
				System.out.println("Device Type: "+"hard coded");
				break;
			case 1:
