...
static int port = 1;	// Synel use 2 port  20170525
...
//	int port = 3;
	...
	len = serial_send_data(port, (char *)packet,sizeof(packet));
//	printf("number of bytes write %d\r\n",len);

	while (1) {
		memset(packet, 0, sizeof(packet));
		len = serial_recv_data(port, packet, sizeof(packet));
		if (len <= 0) {
			continue;
		}
...
//	int port = 3 ;
	...
	while (1) 
	{
		...
		if (ans.flag == SUCCESS) {
			printf("param id 0x%02X\r\n", ans.param);
			printf("param value 0x%02X,%d\r\n", ans.size, ans.size);
			*n = ans.size / 2;		// 0x41 mode, һ��id��������ģ�棬��ʵ����������
			return 1;
		} else if (ans.flag == NOT_FOUND) {
			printf("There is no requested parameter ID found\r\n");
			return 0;
...
//			printf("When there are more users to be enrolled.\r\n");
	...
	while (1) {
		...
		len = serial_recv_data(port, data, data_len);
...
sleep(1);
	...
	{
		memset(data, 0, sizeof(data));
		data_len = data_len1 - data_len2 > sizeof(data) ? sizeof(data) : data_len1 - data_len2;
		len = serial_recv_data(port, data, data_len);
		if (len <= 0) 
		{
			usleep(100000);
...
//	int port = 3;
	...
	printf("enroll pls press finger scan1\n");
...
//	int port = 3;
	...
	printf("enroll pls press finger scan2\n");
...
//	int port = 3;
	...
	printf("enroll pls press finger scan3\n");
...
//			printf("param Image Quality 0x%02X,%d\r\n", ans.size, ans.size);
			j++;
			if (j < 2) {
				printf("enroll pls press finger scan3 again \n");
			}
		} else if (ans.flag == SCAN_SUCCESS) {
//			printf("Fingerprint scanning is successfully  completed\r\n");
			continue;
...
//	int port = 3;
	...
	printf("enroll pls press finger scan4\n");
...
//			printf("param Image Quality 0x%02X,%d\r\n", ans.size, ans.size);
			j++;
			if (j < 2) {
				printf("enroll pls press finger scan4 again \n");
			}
		} else if (ans.flag == SCAN_SUCCESS) {
//			printf("Fingerprint scanning is successfully  completed\r\n");
			continue;
...
//	int port = 3;
	...
	printf("enroll pls press finger scan5\n");
...
//	int port = 3;
	...
	printf("enroll pls press finger scan6\n");
...
//	int port = 3;
	...
	printf("enroll pls press finger scan7\n");
...
//			printf("param Image Quality 0x%02X,%d\r\n", ans.size, ans.size);
			j++;
			if (j < 2) {
				printf("enroll pls press finger scan7 again \n");
			}
		} else if (ans.flag == SCAN_SUCCESS) {
//			printf("Fingerprint scanning is successfully  completed\r\n");
			continue;
...
//	int port = 3;
	...
	printf("enroll pls press finger scan8\n");
...
//			printf("param Image Quality 0x%02X,%d\r\n", ans.size, ans.size);
			j++;
			if (j < 2) {
				printf("enroll pls press finger scan8 again \n");
			}else{
				break;
			}
		} else if (ans.flag == SCAN_SUCCESS) {
...
//	int port = 3;
	...
	printf("enroll pls press finger scan9 \n");
...
//	int port = 3;
	...
	printf("enroll pls press finger scan10\n");
...
//	int port = 3;
	...
	while (1) {
		data_len = data_len1 - data_len2 > sizeof(packet) ?	sizeof(packet) : data_len1 - data_len2;

		memset(packet, 0, sizeof(packet));
		memcpy(packet, ptr, data_len);
		len = serial_send_data(port, (char *)packet, data_len);
	...
	memset(packet, 0, sizeof(packet));
	packet[0] = 0x0A;
	len = serial_send_data(port, (char *)packet, 1);
...
int _identify_by_scan()
{
	...
	if(ans.flag == SCAN_SUCCESS)
	{
//		printf("finger print sucess\n");
		doflag = 1;
//		return -100;
	}
	else if(ans.flag == SUCCESS)
	{
		return ans.param;
	}
	else if(ans.flag == BUSY)
	...
	{
		printf("time out, need send cmd again\n");
		doflag = 0; 
	}
	else if(ans.flag == NOT_FOUND)
	{
...
int sfm_InitFp(int port,int baud_rate) {
	...
	ret = serial_init(port, baud_rate, 1, 1);
	if (ret < 0) {
		printf("sfm_InitFp = %d,%d\n", port, baud_rate);
		return FALSE;
	}
...
int sfm_LoadFpData(char *nID, int FingerNum, char *FileName) {
	...
	memset(zwtemplate, 0, sizeof(zwtemplate));
	ret = fread(zwtemplate, 384, 1, file);		// g_template_size
	if (ret != 1) {
		fclose(file);
		return 0;
	}
...
//			sprintf(file_name,"%s%s_%d.s10",dpath,nID,FingerNum);
			...
			creatdir(file_name);
			printf("save template to file = %s\n", file_name);
...
long sfm_OneToNMatch(char *tpath) {
	...
	id = _identify_by_scan();	
	if (id < 0) {
		printf("_identify_by_scan = %d\n", ret);
		return ret;
	}
...
long sfm_OneToNMatch(char *tpath) 
{
	...
	if(ret > 0 || ret == -100)
	{
		printf("Now to get finger image\n");
		if (tpath) 
		{
			ret = _read_image();
			if (ret == 1) 
